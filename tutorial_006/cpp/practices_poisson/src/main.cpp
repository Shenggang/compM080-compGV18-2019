...
public:
	...
	MyContext() :nv_len(0), point_size(5), line_width(2), mode(0), show_hcolor(0)
	{
	...
	void reset_display(igl::opengl::glfw::Viewer& viewer)
	{
		...
		else if (mode == 1)
		{
			...
			// add mesh
			viewer.data().set_mesh(m_V_solved, m_F); 
			viewer.core.align_camera_center(m_V_solved, m_F); 
		}
...
int main(int argc, char *argv[])
{
	...
	get_example_mesh("camelhead_cut.off", V, F, VN);
	...
	// get boundary vertices for doing visualization
	
	Eigen::MatrixXd vis_bvex;
	
	if (boundary_i.size() >0)
	{
		igl::slice(V, boundary_i, 1, vis_bvex);
	}

	
	std::cout << "call solver\n";

	Eigen::MatrixXd solved_Vex;

	solve_poission_eq(V, F , solved_Vex);
	...
	// for visualization
	g_myctx.m_V = V;
	g_myctx.m_V_solved = solved_Vex;
	g_myctx.m_F = F;
	g_myctx.m_VN = VN;
	//g_myctx.bedges = vis_bedges;
	g_myctx.bvex = vis_bvex;
	...
	menu.callback_draw_custom_window = [&]()
	{
		...
		//mode - List box
		const char* listbox_items[] = { "Input" , "Solved"  };
